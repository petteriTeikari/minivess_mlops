...
def get_dvc_files_of_repo(
    ...
    try:
        ...
        logger.error(
            'Problem getting the DVC cache dir from DVC path "{}", e = {}'.format(
                dvc_repo_path, e
            )
        )
        raise IOError(
    ...
    try:
        debug_dvc_files(
            dvc_repo_path=repo_url, dataset=dataset_name_lowercase, cache_dir=cache_dir
        )
    except Exception as e:
    ...
    local_path = os.path.join(
        repo_dir, fetch_params["datapath_base"], dataset_name_lowercase
    )

    if not os.path.exists(local_path):
        logger.error(
            "Cannot find the dataset from = {} (did you do dvc pull?)".format(
                local_path
            )
        )
        raise IOError(
    ...
    if local_download_duplicate:
        ...
        if skip_already_downloaded:
            ...
            if not download_dvc_data:
                logger.info(
                    'Skipping DVC download, since the data is already downloaded to "{}"'.format(
                        local_path
                    )
                )
        ...
        if download_dvc_data:
            logger.warning(
                "Making a duplicate of the data now, inspect how to improve later "
                "(i.e. using the shared cache)"
            )
            t0 = time.time()
            fs.get(rpath=dvc_dataset_location, lpath=local_path, recursive=True)
            logger.info("Fetch complete in {:.3f} seconds".format(time.time() - t0))

    else:
        raise IOError(
            "You need to download the Minivess with DVC with some method, "
            "when local_download_duplicate=False"
        )


def debug_dvc_files(dvc_repo_path: str, cache_dir: str, dataset: str):
    ...
    try:
        ...
        logger.warning(
            'DVC does not seem to be initialized in the repo? no ".dvc" folder found, e = {}'.format(
                e
            )
        )
    ...
    check_dataset_definition(
        data_path=os.path.join(dvc_repo_path, "data"), dataset=dataset
    )
    ...
    if dvc_config is not None:
        ...
        try:
            ...
            logger.info(
                'Number of cached files = {} ("{}")'.format(
                    no_of_cached_files, cache_dir
                )
            )
        except Exception as e:
            logger.warning(
                'Cannot find cached DVC files from "{}", e = {}'.format(cache_dir, e)
            )


def check_dataset_definition(data_path: str, dataset: str):
    dataset_dvc_file_path = os.path.join(data_path, "{}.dvc".format(dataset))
    if not os.path.exists(dataset_dvc_file_path):
        raise IOError(
            'Cannot find the dataset definition file "{}" for dataset = "{}"'.format(
                dataset_dvc_file_path, dataset
            )
        )
    else:
        ...
        if not os.path.exists(dataset_dir):
            ...
            if dataset == "minivess":
                if "raw" in dataset_dir_filelisting:
                    ...
                    logger.info(
                        'Number of "Raw" files = {} '
                        "(the volumetric 2-PM .nii.gz cubes)".format(
                            len(raw_dir_filelisting)
                        )
                    )
...
def get_dvc_cache_dir(dvc_repo_path: str):
    ...
    if dvc_config is not None:
        ...
        logger.warning(
            'Failed to get the config from "{}", no cache_dir returned")'.format(
                dvc_path
            )
        )
        cache_dir = None
    ...
    try:
        ...
        logger.warning(
            "Running in Colab, quick fix for DVC cache dir = {}".format(cache_dir)
        )
    except:
...
def check_for_dvc_cache_existence(cache_dir: str):
    try:
        ...
        logger.warning(
            "Problem checking for cache dir ({}), e = {}".format(cache_dir, e)
        )
        raise IOError(
            "Problem checking for cache dir ({}), e = {}".format(cache_dir, e)
        )
...
def get_dvc_config(dvc_path: str, config_fname: dict = "config"):
    import configparser
    ...
    if not os.path.exists(dvc_path):
        logger.error(
            'Cannot find DVC path in the repo "{}", '
        ...
        try:
            ...
            logger.error(
                "Failed to list the files in base path of the repo, e = {}".format(e)
            )
