...
from src.inference.ensemble_utils import (
    add_sample_results_to_ensemble_results,
    add_sample_metrics_to_split_results,
...
def inference_ensemble_dataloader(
    models_of_ensemble: dict,
    cfg: dict,
    split: str,
    ...
    ensemble_results = inference_ensemble_with_dataloader(
        ensemble_model, dataloader=dataloader, split=split
    )

    return ensemble_results


def inference_ensemble_with_dataloader(ensemble_model, dataloader, split: str):
    with torch.no_grad():
        dataloader_metrics = {}
        for batch_idx, batch_data in enumerate(
            tqdm(
                dataloader,
                desc='ENSEMBLE: Inference on dataloader samples, split "{}"'.format(
                    split
                ),
                position=0,
            )
            ...
            dataloader_metrics = add_sample_metrics_to_split_results(
                sample_metrics=deepcopy(sample_ensemble_metrics),
    ...
    dataloader_metrics_stat = compute_split_metric_stats(
...
def ensemble_repeats(
    ...
    if var_type_key in inf_res:
        if var_key in inf_res[var_type_key]:
            ...
            logger.error(
                "var_key = {}, not found in the var_type_key = {}, keys = {}".format(
                    var_key, var_type_key, inf_res[var_type_key].keys()
                )
            )
            raise IOError(
                "var_key = {}, not found in the var_type_key = {}, keys = {}".format(
        ...
        logger.error(
            "var_type_key = {}, not in the inference results, keys = {}".format(
                var_type_key, inf_res.keys()
            )
        )
        raise IOError(
            "var_type_key = {}, not in the inference results, keys = {}".format(
...
def compute_ensembled_response(input_data: np.ndarray, ensemble_params) -> dict:
    assert len(input_data.shape) == 6, (
        "Input should be 6D (no_submodels,B,C,H,W,D), " "now it is {}D"
    ).format(len(input_data))

    variable_stats = {"scalars": {}}
    variable_stats["scalars"]["n_samples"] = input_data.shape[
        0
    ]  # i.e. number of repeats / submodels

    variable_stats = {"arrays": {}}
    variable_stats["arrays"]["mean"] = np.mean(
        input_data, axis=0
    )  # i.e. number of repeats / submodels
    variable_stats["arrays"]["var"] = np.var(
        input_data, axis=0
    )  # i.e. number of repeats / submodels
    # TODO! UQ
    ...
    if ensemble_params["method"] == "average":
        variable_stats["arrays"]["mask"] = (
            variable_stats["arrays"]["mean"] > ensemble_params["mask_threshold"]
        ).astype("float32")
    else:
        # majority_voting_here
        raise NotImplementedError(
            "Unknown ensemble method = {}".format(ensemble_params["method"])
        )
...
class ModelEnsemble(mlflow.pyfunc.PythonModel):
    ...
    def __init__(
        ...
        precision: str,
        eval_config: dict,
        model_best_dicts: dict = None,
        models_from_paths: bool = True,
        mode: str = "evaluate",
    ):
        ...
        for submodel_name in models_of_ensemble:
            if models_from_paths:
                ...
                (
                    self.models[submodel_name],
                    self.model_best_dicts[submodel_name],
                    _,
                    _,
                ) = import_model_from_path(
                    model_path=models_of_ensemble[submodel_name],
                ...
                if len(self.model_best_dicts) == 0:
                    raise IOError(
                        "Your best_dicts is empty, refactor this to be smarter,"
            ...
            if mode == "evaluate":
                ...
                raise NotImplementedError(
                    "Only evaluation defined, fix here if you want to resume training"
                )

    def __inference_per_sample(self, input_data, input_from: str = "dict"):
        # Inference the batch_data through all the submodels
        inference_results = {}
        for submodel_name in self.models:
            model = self.models[submodel_name]
            monai_metric_dict = get_monai_param_dict_from_omegaconf(
                self.validation_params, model
            )
            sample_res = inference_sample(
                input_data,
            ...
            inference_results = add_sample_results_to_ensemble_results(
                inf_res=deepcopy(inference_results), sample_res=sample_res
            )
    ...
    def predict(
        self, context, model_input: Union[np.ndarray, torch.Tensor], param: dict = None
    ):
        """
        ...
        inference_results = self.__inference_per_sample(
            input_data=image_tensor, input_from="tensor"
        )

        ensemble_stat_results = ensemble_repeats(
            inf_res=inference_results, ensemble_params=self.ensemble_params
        )
    ...
    def predict_with_gt(
        self, batch_data, input_from: str = "dict", return_mask: bool = False
    ):
        """
        ...
        """
        inference_results = self.__inference_per_sample(
            input_data=batch_data, input_from="dict"
        )
    ...
    def get_model_weights(self):
        ...
        for ensemble_name in self.models:
            ...
            weights = get_last_layer_weights_of_model(
                model, p_weights=1.00, layer_name_wildcard="conv"
            )
