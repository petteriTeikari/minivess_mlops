...
def get_sample_metrics_from_np_masks(x: np.ndarray, y: np.ndarray, y_pred: np.ndarray,
    ...
    if 'hausdorff' in eval_config['METRICS']:
        ...
        sample_metrics['metrics'][metric] = \
            (compute_hausdorff_distance(y_pred=y_pred_onehot, y=y_onehot,
                                        include_background=include_background).detach().squeeze().numpy())
    ...
    if 'dice' in eval_config['METRICS']:
        ...
        metric = 'dice'
        sample_metrics['metrics'][metric] = \
            (compute_generalized_dice(y_pred=y_pred_onehot, y=y_onehot,
                                      include_background=include_background).detach().numpy())
        sample_metrics['timing'][metric] = np.array([time.time() - t0])
        metrics_computed += 1
...
def prepare_for_metrics(y, y_pred, x):
    ...
    assert len(y.shape) == 5, 'Input should be 5D (BCHWD), now it is {}D'.format(len(y.shape))
    ...
    assert len(y_onehot.shape) == 6, 'y_onehot should be 6D (B,HotEncClass,C,H,W,D), now it is {}D'.format(len(y_onehot))
