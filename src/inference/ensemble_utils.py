...
def add_sample_results_to_ensemble_results(inf_res: dict, sample_res: dict):
    for var_type in sample_res:
        ...
        for var in sample_res[var_type]:
            ...
            if len(inf_res[var_type][var]) == 0:
                ...
                inf_res[var_type][var] = np.concatenate(
                    (inf_res[var_type][var], sample_expanded), axis=0
                )

    return inf_res


def add_sample_metrics_to_split_results(
    sample_metrics: dict, dataloader_metrics_tmp: dict
) -> dict:
    ...
    for var_type in sample_metrics:
        ...
        for var in sample_metrics[var_type]:
            sample_metric_expanded = np.expand_dims(
                sample_metrics[var_type][var], axis=0
            )
            if is_first_sample:
                dataloader_metrics_tmp[var_type][var] = sample_metric_expanded
            else:
                dataloader_metrics_tmp[var_type][var] = np.concatenate(
                    (dataloader_metrics_tmp[var_type][var], sample_metric_expanded),
                    axis=0,
                )
...
def get_metadata_for_sample_metrics(metadata: dict) -> dict:
    ...
    try:
        ...
        sample_metadata = {"filepath_json": filepath, "sample_name": sample_name}
    except Exception as e:
        logger.warning(
            'Problem getting the metadata? error = "{}",\n'
            "This tested now only for MINIVESS that has the filepath in the metadata".format(
                e
            )
        )
...
def compute_split_metric_stats(
    dataloader_metrics: dict, var_types_to_keep: tuple = ("metrics", "timing")
) -> dict:
    dataloader_stats = {}

    for var_type in dataloader_metrics:
        if (
            var_type in var_types_to_keep
        ):  # hard to compute stats for metadata for example
            ...
            for var in dataloader_metrics[var_type]:
                ...
                dataloader_stats[var_type][var] = compute_stats_of_array_in_dict(
                    input_data
                )
...
def get_ensemble_name(dataset_validated: str, metric_to_track: str) -> str:
...
def get_submodel_name(
    repeat_name: str, archi_name: str = None, fold_name: str = None
) -> str:
