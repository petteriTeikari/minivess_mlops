        ...
        if split in allowed_splits:
            ...
            if isinstance(dataloaders[split], dict):
                ...
                for dataset_name in dataloaders[split]:
                    ...
                    dataloader = redefine_dataloader_for_inference(
                        dataloader_batched=dataloader_batched,
                    ...
                    if model_scheme == "ensemble_from_repeats":
                        results_out[split] = inference_ensembles_dataloader(
                        ...
                        results_out[split] = inference_best_repeat(
                            dataloader=dataloader,
                            split=split,
                ...
                dataloader = redefine_dataloader_for_inference(
                    dataloader_batched=dataloaders[split],
                    dataset_name=dataset_name,
                    split=split,
                    device=device,
                    cfg=cfg,
...
def inference_ensembles_dataloader(
    dataloader, split: str, archi_results: dict, cfg: dict, device: str
):
    ...
    logger.info(
        "No of submodels in ensemble = {} ({} repeats, {} architectures)".format(
            no_submodels, len(repeat_names), len(architecture_names)
        )
    )
    logger.info(
        "No of ensembles = {} ({} eval_datasets, {} tracked metrics)".format(
            no_ensembles, no_eval_datasets, no_tracked_metrics
        )
    )
    ...
    for i, ensemble_name in enumerate(ensemble_models_flat):
        logger.info(
            'Inference on ensemble_name = "{}" (#{}/{})'.format(
                ensemble_name, i + 1, len(ensemble_models_flat)
            )
        )
        ensemble_results[ensemble_name] = inference_ensemble_dataloader(
...
def collect_submodels_of_the_ensemble_archi(archi_results: dict):
            ...
            for ds in best_dict:
                ...
                for tracked_metric in best_dict[ds]:
                    ...
                    submodel_name = get_submodel_name(
                        archi_name=archi_name, repeat_name=repeat_name
                    )
    ...
    logger.info(
        "Collected a total of {} models, and {} distinct ensembles for ensemble inference".format(
