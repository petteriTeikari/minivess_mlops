...
def define_transforms(dataset_config: dict,
    ...
    for i, split in enumerate(transform_config_per_dataset.keys()):
        transforms[split] = get_transforms_per_split(transform_cfg_name_per_split = transform_config_per_dataset[split],
                                                     split = split,
...
def get_transforms_per_split(transform_cfg_name_per_split: str, split: str = 'TRAIN', transform_config: dict = None):

    if transform_cfg_name_per_split == 'BASIC_AUG':
        transforms = basic_aug() # eliminate the if/elif/else and parse directly from str?
