...
def define_datasets(
    cfg: dict, fold_split_file_dicts: dict, debug_testing: bool = False
):
    """
    ...
    if debug_testing:
        logger.warning(
            "WARNING! Debugging mode on for dataset, and errors are intentionally added!"
        )
    ...
    for f, fold_name in enumerate(fold_split_file_dicts):
        ...
        for i, dataset_name in enumerate(fold_split_file_dicts[fold_name].keys()):
            logger.info(
                'Creating (MONAI/PyTorch) Datasets for dataset source = "{}"',
                dataset_name,
            )
            dataset_config = cfg_key(
                cfg, "hydra_cfg", "config", "DATA", "DATA_SOURCE", dataset_name
            )
            ...
            (
                ...
                ml_test_dataset[fold_name][dataset_name],
            ) = define_torch_dataset(
    ...
    if not all_tests_ok:
        print(report_string)
        logger.debug(
            'Your chance to write the dataset ML test "report_to_string" to a file or something'
        )
...
def redefine_dataloader_for_inference(
    dataloader_batched,
    cfg: dict,
    ...
    def redefine_dataset_for_inference(
        dataloader_batched, dataset_name: str, device: str
    ):
        ...
        if dataset_name == "MINIVESS":
            dataset = Dataset(data=split_file_dict, transform=transforms)
        else:
            raise NotImplementedError(
                'Only implemented minivess dataset now!, not = "{}"'.format(
                    cfg_key(cfg, "hydra_cfg", "config", "DATA", "DATASET_NAME")
                )
            )
    ...
    logger.info(
        "Redefining MONAI dataset/dataloader for inference (batch size = 1, original resolution), "
...
def define_dataset_and_dataloader(cfg: dict, fold_split_file_dicts: dict):
    datasets = define_datasets(
        cfg=cfg,
        fold_split_file_dicts=fold_split_file_dicts,
        debug_testing=cfg_key(
            cfg, "hydra_cfg", "config", "TESTING", "DATASET", "debug_testing"
        ),
    )

    dataloaders = define_dataloaders(
        datasets=datasets,
        cfg=cfg,
        dataloader_config=cfg_key(cfg, "hydra_cfg", "config", "DATA", "DATALOADER"),
    )
    ...
    datasets, dataloaders = quick_and_dirty_training_dataloader_creation(
    ...
    if cfg_key(
        cfg, "hydra_cfg", "config", "TESTING", "DATALOADER", "DATA_VALIDITY", "enable"
    ):
        ml_test_dataloader_dict_integrity(
            dataloaders,
            test_config=cfg_key(cfg, "hydra_cfg", "config", "TESTING", "DATALOADER"),
            run_params=cfg_key(cfg, "run", "PARAMS"),
            debug_testing=cfg_key(
                cfg, "hydra_cfg", "config", "TESTING", "DATALOADER", "debug_testing"
            ),
        )
    else:
...
def quick_and_dirty_training_dataloader_creation(
    datasets, dataloaders, dataset_name: str = "MINIVESS", fold_name: str = "fold0"
):
    ...
    if dataset_name in datasets[fold_name]:
        logger.info(
            "Quick and dirty fixing of MINIVESS dataset to the desired format of 1 TRN dataloader, "
        ...
        dataloaders_out[fold_name]["TRAIN"] = dataloaders[fold_name][dataset_name][
            "TRAIN"
        ]
        ...
        dataloaders_out[fold_name]["VAL"] = {
            dataset_name: dataloaders[fold_name][dataset_name]["VAL"]
        }
        dataloaders_out[fold_name]["TEST"] = {
            dataset_name: dataloaders[fold_name][dataset_name]["TEST"]
        }
    else:
        logger.info(
            'No quick asnd dirty fix for dataset_name = "{}"'.format(dataset_name)
        )
...
def define_dataloaders(datasets: dict, cfg: dict, dataloader_config: dict):
    ...
    for f, fold_name in enumerate(datasets.keys()):
        ...
        for i, dataset_name in enumerate(datasets[fold_name].keys()):
            ...
            for j, split in enumerate(dataset_per_name.keys()):
                logger.info(
                    "Dataloader for fold = {}, dataset = {}, split = {}, batch_sz = {}, num_workers = {}".format(
                        fold_name,
                        dataset_name,
                        split,
                ...
                dataloaders[fold_name][dataset_name][split] = DataLoader(
                    dataset_per_name[split],
