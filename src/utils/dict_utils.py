    ...
    for key in keys:
        ...
        try:
            ...
            if mode == 'NoneReturn':
                ...
                logger.error('Problem getting the key "{}" from the config\n{}'.format(keys, dct))
                raise KeyError('Problem getting the key "{}" from the config\n{}'.format(keys, dct))
            else:
...
def safe_assign_to_dict(dct: dict,
                        dict_add: dict,
                        keys_to_check: tuple,
                        key_to_add: str):
    ...
    if len(keys_to_check) == 1:
        ...
        dct = safe_assign_to_dict(dct[keys_to_check[0]],
                                  dict_add,
                                  keys_to_check,
                                  key_to_add)
...
def put_to_dict(dct, dict_add, *keys,
                create_parent_keys: bool = False):

    if len(keys) == 0:
        logger.error('You need to provide keys when trying to add:\n{}\nto\n{}'.format(dict_add, dct))
        raise IOError('You need to provide keys when trying to add:\n{}\nto\n{}'.format(dict_add, dct))
    ...
    if len(keys) == 1:
        ...
        if parent_dict_exists:
            ...
            if create_parent_keys:
                ...
                logger.error('You tried to add a key "{}" to the dictionary, '
                             'but the parent key "{}" does not exist!\n'.format(key_to_add, keys_to_check))
                raise KeyError('You tried to add a key "{}" to the dictionary, '
                               'but the parent key "{}" does not exist!\n'.format(key_to_add, keys_to_check))
