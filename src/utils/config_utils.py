...
def get_hydra_config_paths(
    task_cfg_name: str = None,
    ...
    def get_config_path():
        ...
        if not os.path.exists(absolute_config_dir):
            logger.error(
                'Configuration dir "{}" does not exist!'.format(absolute_config_dir)
            )
            raise IOError(
                'Configuration dir "{}" does not exist!'.format(absolute_config_dir)
            )
    ...
    def get_task_cfg_overrides_list(task_cfg_name: str, abs_cfg_path: str):
        if task_cfg_name is not None:
            ...
            if len(subdir) == 0:
                logger.error(
                    'No subdir of the config detected in "{}" (should be like "train/cfg")'.format(
                        task_cfg_name
                    )
                )
                raise IOError(
    ...
    if reprod_cfg_name is not None:
        logger.info(
            'You had provided "reprod_cfg" = "{}"! '
            'Using the config as it is for reproduction of previous run without "overrides"'.format(
                reprod_cfg_name
            )
        )
        ...
        overrides_list, abs_task_cfg_path = get_task_cfg_overrides_list(
            task_cfg_name=task_cfg_name, abs_cfg_path=abs_cfg_path
        )
...
def update_task_cfg_for_ci_cd_jobs(
    run_mode: str = "test_dataload", cicd_dir_name: str = "runmode_configs"
) -> str:
    task_cfg_name = f"{cicd_dir_name}/{run_mode}_config"
    logger.info(
        'Run mode = "{}", updating the task config to "{}"'.format(
            run_mode, task_cfg_name
        )
    )
    return task_cfg_name


def hydra_import_config(
    hydra_cfg_paths: dict,
    ...
    try:
        ...
        with initialize(
            config_path=os.path.join(
                parent_dir_string_defaults, hydra_cfg_paths["config_path"]
            ),
            job_name=job_name,
            version_base=hydra_version_base,
        ):
            cfg = compose(
                config_name=base_cfg_name, overrides=hydra_cfg_paths["overrides_list"]
            )
            logger.info(
                'Initializing Hydra with config_path = "{}", job_name = "{}", version_base = "{}"'.format(
                    hydra_cfg_paths["abs_cfg_path"], job_name, hydra_version_base
                )
            )
            logger.info(
                "Hydra overrides list = {}".format(hydra_cfg_paths["overrides_list"])
            )
...
def config_import_script(
    ...
    hydra_cfg_paths = get_hydra_config_paths(
        task_cfg_name=task_cfg_name,
    ...
    config = hydra_import_config(
        hydra_cfg_paths=hydra_cfg_paths,
        job_name=job_name,
...
def import_config(
    ...
    hydra_cfg = config_import_script(
        task_cfg_name=task_cfg_name,
        reprod_cfg_name=reprod_cfg_name,
        job_name=args["project_name"],
    )

    run_params = set_up_experiment_run(
        hydra_cfg=hydra_cfg, args=args, log_level=log_level
    )

    return {"hydra_cfg": hydra_cfg, "run": run_params}


def set_up_experiment_run(
    hydra_cfg: DictConfig, args: dict, log_level: str = "DEBUG"
) -> dict:
    ...
    machine_dict = set_up_environment(
        machine_config=cfg_key(hydra_cfg, "config", "MACHINE"),
    ...
    if args["s3_mount"]:
        raise NotImplementedError(
            "The real-time write to S3 seems like a dream at the moment, so aws sync at the end"
        )
    ...
    run_params["PARAMS"] = set_up_run_params(
        hydra_cfg=hydra_cfg, args=args, hyperparam_name=hyperparam_name
    )

    # Define hyperparameters for Experimnent Tracking (MLflow/WANDB)
    (
        run_params["HYPERPARAMETERS"],
        run_params["HYPERPARAMETERS_FLAT"],
    ...
    if cfg_key(hydra_cfg, "config", "LOGGING", "unique_hyperparam_name_with_hash"):
        run_params["PARAMS"] = use_dict_hash_in_names(
            params=cfg_key(run_params, "PARAMS"),
            config_hash=cfg_key(run_params, "PARAMS", "config_hash"),
        )

    # Set-up the log files (loguru, stdout/stderr)
    run_params = set_up_log_files(
        run_params=run_params, hyperparam_name=hyperparam_name, log_level=log_level
    )
    ...
    run_params["MLFLOW"], mlflow_dict = init_mlflow_logging(
        hydra_cfg=hydra_cfg,
...
def get_repo_base_dir(working_dir: str, repo_names: tuple = ("minivess_mlops", "app")):
    ...
    if os.path.split(working_dir)[-1] in repo_names:
        ...
        if os.path.split(working_dir)[-1] in repo_names:
            ...
            if os.path.split(working_dir)[-1] in repo_names:
                ...
                logger.error(
                    "Could not find the repo base dir, from {}".format(working_dir_orig)
                )
                raise IOError(
                    "Could not find the repo base dir, from {}".format(working_dir_orig)
                )
...
def get_repo_dir(
    ...
    cwd = os.getcwd()
    repo_dir = get_repo_base_dir(working_dir=cwd, repo_names=repo_names)
    if verbose:
...
def set_up_run_params(hydra_cfg: DictConfig, args: dict, hyperparam_name: str):
    # Create hash from config dict
    config_hash = dict_hash(
        dictionary=deepcopy(hydra_cfg["config"]), drop_keys=("SERVICES",)
    )
    ...
    run_params = {
        ...
        "output_experiment_dir": os.path.join(
            output_experiments_base_dir, hyperparam_name
        ),
    ...
    run_params["requirements-txt_path"] = get_requirements_txt_path(
        repo_dir=run_params["repo_dir"]
    )
...
def get_requirements_txt_path(repo_dir: str, raise_error: bool = True) -> str:
    ...
    if not os.path.exists(requirements_path):
        ...
        logger.error(
            'Could not find requirements.txt from "{}"\n'
        ...
        if raise_error:
            ...
            logger.info(
                "Setting the requirements.txt path to None! \n"
...
def use_dict_hash_in_names(params: dict, config_hash: str) -> dict:
    ...
    logger.info(
        '  output_experiment_dir = "{}"'.format(params["output_experiment_dir"])
    )
...
def define_hyperparams_from_config(hydra_cfg: DictConfig):
    ...
    }  # flatten_nested_dictionary(dict_in=config['hyperparameters'])
    logger.info('Save the derived hyperparameters to config["hyperparameters"]')
    logger.info(
        hyperparameters
    )  # TODO add to print_the_dict_to_logger() nested dicts as well

    return hyperparameters, hyperparameters_flat


def set_up_log_files(run_params: dict, hyperparam_name: str, log_level: str = "DEBUG"):
    log_format = (
    ...
    run_params["PARAMS"]["output_log_path"] = os.path.join(
        run_params["PARAMS"]["output_experiment_dir"], log_file
    )
    try:
        logger.add(
            run_params["PARAMS"]["output_log_path"],
            level=log_level,
            format=log_format,
            colorize=False,
            backtrace=True,
        ...
        logger.error(
            "Problem initializing the log file to the artifacts output, permission issues?? e = {}".format(
                e
            )
        )
        raise IOError(
            "Problem initializing the log file to the artifacts output, have you created one? "
            'do you have the permissions correct? See README.md for the "minivess_mlops_artifacts" creation'
            "with symlink to /mnt \n error msg = {}".format(e)
        )
    logger.info(
        'Log (loguru) will be saved to disk to "{}"'.format(
            run_params["PARAMS"]["output_log_path"]
        )
    )

    log_file = "stdout_{}.txt".format(hyperparam_name)
    run_params["PARAMS"]["stdout_log_path"] = os.path.join(
        run_params["PARAMS"]["output_experiment_dir"], log_file
    )
    logger.info(
        'Stdout will be saved to disk to "{}"'.format(
            run_params["PARAMS"]["stdout_log_path"]
        )
    )
...
def set_up_environment(machine_config: DictConfig, local_rank: int):
    ...
    machine_config = {"device": device, "local_rank": local_rank}

    return machine_config


def dict_hash(dictionary: Dict[str, Any], drop_keys: tuple) -> str:
    """
    ...
    for drop_key in drop_keys:
        ...
        logger.debug(
            'Dropping "{}" from the dictionary before computing hash'.format(drop_key)
        )
    ...
    try:
        encoded = json.dumps(
            dictionary, sort_keys=True, default=to_serializable
        ).encode()
        dhash.update(encoded)
        hash_out = dhash.hexdigest()
    except Exception as e:
        logger.warning(
            "Problem getting the hash of the config dictionary, error = {}".format(e)
        )
        hash_out = None
    return hash_out
...
def define_hyperparam_run_params(hydra_cfg: DictConfig) -> dict:
    ...
    logger.info(
        'Hand-picking the keys/subdicts from "config" that are logged as hyperparameters for MLflow/WANDB'
    )
    ...
    if training_tmp["SKIP_TRAINING"]:
        ...
        for model_name in model_names:
            ...
            for setting_name in setting_names:
                try:
                    settings, param_name = parse_settings_by_name(
                        model_cfg=training_tmp[model_name][setting_name]
                    )
                    hyperparams["models"][model_name]["training_extra"][
                        setting_name
                    ] = {}
                    ...
                    logger.error(
                        "Problem parsing the training settings, model_name = {}, setting_name = {}, e = {}".format(
                            model_name, setting_name, e
                        )
                    )
                    raise IOError(
                        "Problem parsing the training settings, model_name = {}, setting_name = {}, e = {}".format(
...
def get_default_cfg(
    parent_dir_string: str = os.path.join("..", ".."),
    parent_dir_string_defaults: str = ".",
):
...
def get_service_uris(
    cfg_yaml: str = "defaults.yaml",
    ...
    hydra_cfg: DictConfig = get_default_cfg(
        parent_dir_string=parent_dir_string,
        parent_dir_string_defaults=parent_dir_string_defaults,
    )
