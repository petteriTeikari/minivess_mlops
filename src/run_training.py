...
def parse_args_to_dict():
    parser = argparse.ArgumentParser(
        description="Segmentation pipeline for Minivess dataset"
    )
    parser.add_argument(
        "-c",
        "--task_config-file",
    ...
    parser.add_argument(
        ...
        required=False,
        default="train",
        choices=["train", "debug", "test_data", "test_dataload", "test_train"],
    ...
    parser.add_argument(
        "-no-s3", "--no-s3_mount", dest="s3_mount", action="store_false"
    )
...
if __name__ == "__main__":
    ...
    for hyperparam_idx, hyperparam_name in enumerate(hyperparam_runs):
        logger.info(
            "Starting hyperparam run {}/{}: {}".format(
                hyperparam_idx + 1, len(hyperparam_runs), hyperparam_name
            )
        )
    ...
    if args["run_mode"] == "train":
        ...
        if not cfg_key(
            hparam_run_results[hyperparam_name]["cfg"],
            "hydra_cfg",
            "config",
            "TRAINING",
            "SKIP_TRAINING",
        ):
            mlflow_update_best_model(
