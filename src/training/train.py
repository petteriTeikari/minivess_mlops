...
from src.log_ML.logging_main import (
    log_epoch_results,
    log_n_epochs_results,
    log_crossvalidation_results,
    log_averaged_and_best_repeats,
)
...
def training_script(
    ...
    logger.info(
        'Starting training for the hyperparameter config "{}"'.format(hyperparam_name)
    )
    ...
    # TODO! put_to_dict()
    cfg["run"]["PARAMS"]["cross_validation"] = os.path.join(
        output_dir, "cross_validation"
    )
    cfg["run"]["PARAMS"]["cross_validation_averaged"] = os.path.join(
        cfg["run"]["PARAMS"]["cross_validation"], "averaged"
    )
    cfg["run"]["PARAMS"]["cross_validation_ensembled"] = os.path.join(
        cfg["run"]["PARAMS"]["cross_validation"], "ensembled"
    )

    for f, fold_name in enumerate(list(experim_dataloaders.keys())):
        logger.info(
            "Training fold #{}/{}: {}".format(
                f + 1, len(experim_dataloaders.keys()), fold_name
            )
        )
        # TODO! put_to_dict()
        cfg["run"]["PARAMS"]["fold_dir"][fold_name] = os.path.join(
            output_dir, fold_name
        )
        cfg["run"]["PARAMS"]["ensemble_artifacts"][fold_name] = os.path.join(
            cfg["run"]["PARAMS"]["fold_dir"][fold_name], "ensemble"
        )
        ...
        (
            ...
            ensembled_results[fold_name],
        ) = train_model_for_single_fold(
            fold_dataloaders=experim_dataloaders[fold_name],
            cfg=cfg,
    ...
    cfg["run"]["PARAMS"]["logged_model_paths"] = log_crossvalidation_results(
...
def train_model_for_single_fold(
    fold_dataloaders: dict,
    cfg: dict,
    ...
    for i, architecture_name in enumerate(architecture_names):
        logger.info(
            "Training architecture #{}/{}: {}".format(
                i + 1, len(architecture_names), architecture_name
            )
        )
        ...
        try:
            training_config = cfg_key(
                hyperparameters_config, "models", architecture_name, "training"
            )
            model_config = cfg_key(
                hyperparameters_config, "models", architecture_name, "architecture"
            )
    ...
    if cfg_key(cfg, "hydra_cfg", "config", "ENSEMBLE", "enable"):
        artifacts_out_dir = cfg_key(
            cfg, "run", "PARAMS", "ensemble_artifacts", fold_name
        )
        ensemble_results = reinference_dataloaders(
            input_dict=archi_results,
        ...
        log_ensemble_results(
            ensemble_results, cfg=cfg, fold_name=fold_name, service=service
        )
...
def train_model_for_single_architecture(
    fold_dataloaders: dict,
    cfg: dict,
    ...
    for repeat_idx in range(training_config["NO_REPEATS"]):
        logger.info(
            "Training repeat #{}/{}".format(
                repeat_idx + 1, training_config["NO_REPEATS"]
            )
        )
        repeat_name = "repeat{}".format(str(repeat_idx + 1).zfill(2))
        submodel_name = get_submodel_name(
            repeat_name=repeat_name, archi_name=archi_name
        )
        repeat_results[repeat_name] = train_single_model(
            dataloaders=fold_dataloaders,
            cfg=cfg,
            training_config=training_config,
            model_config=model_config,
    ...
    logger.info(
        'Done training all the {} repeats of "{}"'.format(
            training_config["NO_REPEATS"], fold_name
        )
    )
...
def train_single_model(
    ...
    loss_function, optimizer, lr_scheduler = set_model_training_params(
        model=model,
        device=device,
        scaler=scaler,
        training_config=training_config,
    ...
    # Train script
    train_results, eval_results, best_dict, output_artifacts = train_n_epochs_script(
        model=model,
...
def train_n_epochs_script(
    ...
    print(" ")
    for epoch in tqdm(
        range(start_epoch, training_config["NUM_EPOCHS"]),
        desc="Training the network, {}, repeat {}, epoch#".format(
            fold_name, repeat_idx + 1
        ),
        position=0,
    ):
        eval_epoch_results = {}
        ...
        if epoch == start_epoch:
            model_test_metrics0, model_tests0 = model_tests_main(
                model,
                initial_model=None,
        ...
        # Train
        train_epoch_results, eval_epoch_results["TRAIN"] = train_1_epoch(
            model,
            device,
            epoch,
            loss_function,
        ...
        # ML Tests (again)
        model_test_metrics, model_tests = model_tests_main(
            model,
            initial_model=None,
            test_config=cfg_key(cfg, "hydra_cfg", "config", "TESTING", "MODEL"),
            first_epoch=True,
        ...
        train_results, eval_results = collect_epoch_results(
        ...
        output_artifacts = log_epoch_results(
            train_epoch_results=train_epoch_results,
            eval_epoch_results=eval_epoch_results,
            epoch=epoch,
            cfg=cfg,
        ...
        best_dicts = save_models_if_improved(
...
def train_1_epoch(
    ...
    model.train()
    epoch_eval_res = init_epoch_dict(
        epoch, loaders={dataset_dummy_key: train_loader}, split_name="TRAIN"
    )
    epoch_trn_res = init_epoch_dict(
        epoch, loaders={dataset_dummy_key: train_loader}, split_name="TRAIN"
    )
    batch_losses = []
    batch_szs = []
    ...
    epoch_trn_res[dataset_dummy_key]["metadata_scalars"][
        "lr"
    ] = lr_scheduler.get_last_lr()[0]
    epoch_trn_res[dataset_dummy_key] = get_timings_per_epoch(
        metadata_dict=epoch_trn_res[dataset_dummy_key],
...
def train_1_batch(model, device, batch_data, loss_function, amp_on: bool = True):
    if amp_on:
        with torch.cuda.amp.autocast():
            ...
            )  # .to(device) here instead of "ToDeviced"
            # "ToDeviced" lead to GPU memory glitches, inspect this later?
            loss = loss_function(
                outputs, batch_data["label"].to(device)
            )  # .to(device) here instead of "ToDeviced"
    else:
        outputs = model(
            batch_data["image"].to(device)
