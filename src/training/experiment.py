...
def define_experiment_data(cfg: dict):
    ...
    fold_split_file_dicts, cfg['run'] = \
        import_datasets(data_cfg=cfg_key(cfg, 'hydra_cfg', 'config', 'DATA'),
                        cfg=cfg,
    ...
    if not cfg_key(cfg, 'hydra_cfg', 'config', 'DATA', 'DATALOADER', 'SKIP_DATALOADER'):
        experim_datasets, experim_dataloaders = \
            define_dataset_and_dataloader(cfg=cfg,
                                          fold_split_file_dicts=fold_split_file_dicts)
    else:
        # If you are running Github Action for checking Dataset integrity
        logger.warning('Skip dataloader creation, run_mode = "{}"'.format(cfg_key(cfg, 'run', 'ARGS', 'run_mode')))
...
def run_train_experiment(cfg: dict,
                         hyperparam_name: str):

    # Define the used data
    fold_split_file_dicts, experim_datasets, experim_dataloaders, cfg['run'] = (
