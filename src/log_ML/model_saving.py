        ...
        for dataset in eval_epoch_results["VAL"].keys():
            ...
            for m_idx, metric in enumerate(validation_config["METRICS_TO_TRACK"]):
                ...
                if best_dicts is None:
                    best_dicts_out[dataset][
                        metric
                    ] = get_best_dict_from_current_epoch_results(
                        eval_epoch_results, train_epoch_results
                    )
                    ...
                    model_dict = model_improved_script(
                        best_dict=best_dicts_out[dataset][metric],
                ...
                # After 1st epoch, when you have something on your best dict
                    ...
                    if model_improved:
                        ...
                        best_dicts_out[dataset][metric] = deepcopy(
                            best_dicts[dataset][metric]
                        )

    else:
        logger.debug(
            "epoch {} not past warmup epochs ({}) "
            "yet -> not tracking model improvement yet".format(
                epoch, validation_config["NO_WARMUP_EPOCHS"]
            )
        )
...
def check_if_value_improved(
    ...
    best_value_so_far = best_dict["eval_epoch_results"][split_key][dataset][
        results_type
    ][metric]
...
def model_improved_script(
    ...
    best_dict["weights_vector"] = get_last_layer_weights_of_model(
        model, p_weights=1.00, layer_name_wildcard="conv"
    )

    model_fname_base = "bestModel__{}__{}__{}__{}".format(
        dataset, metric, fold_name, repeat_name
    )
    ...
    try:
        ...
        logger.warning(
            "Problem getting model file size from disk to display, e = {}".format(e)
        )
        filesize = np.nan

    logger.debug(
        'epoch #{} | "{}" improved from {:.4f} to {:.4f} (dataset = {}) '
        "-> saving this to disk (model size = {:.1f} MB)".format(
            epoch + 1, metric, best_value_so_far, current_value, dataset, filesize
        )
    )
...
def import_model_from_path(
    model_path: str, validation_config: dict, verbose: bool = False
):
    if os.path.exists(model_path):
        ...
        if validation_config["SAVE_FULL_MODEL"]:
            ...
            raise NotImplementedError(
                "Inference implemented only now for full model saving,\n"
                'not for "just weights" saving'
            )

    else:
        raise IOError(
            "The model file does not exist on disk? (path = {})\n"
            "This should not really happen as we managed to save these during training".format(
                model_path
            )
        )
...
def get_best_dict_from_current_epoch_results(
    eval_epoch_results: dict, train_epoch_results: dict
):
