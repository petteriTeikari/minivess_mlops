        ...
        for metric_name in metric_dict.keys():
            if metric_type == "train":
                if "metadata" in var_type:
                    ...
                    metadata_key = ""
                metric_key_out = (
                    metric_type + metadata_key + "/" + dataset_name + "/" + metric_name
                )
            else:
                ...
                if "metadata" in var_type:
                    ...
                    metadata_key = ""
                metric_key_out = (
                    metadata_key + metric_name + "/" + split_name + "/" + dataset_name
                )

            if multiple_values_per_epoch:
                # you are writing a array output (array per epoch)
                no_values_per_metric = len(
                    metric_dict[metric_name]
                )  # e.g. number of batches per epoch
                for b in range(no_values_per_metric):
                    idx = (epoch * no_values_per_metric) + b
                    (
                        output_artifacts["epoch_level"]["tb_writer"].add_scalar(
                            metric_key_out, metric_dict[metric_name][b], idx
                        )
                    )
            else:
                ...
                (
                    output_artifacts["epoch_level"]["tb_writer"].add_scalar(
                        metric_key_out, metric_dict[metric_name], epoch
                    )
                )
