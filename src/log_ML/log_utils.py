...
def get_number_of_steps_from_repeat_results(
    results: dict, result_type: str = "train_results"
):
    no_steps = np.nan
    try:
        ...
        logger.warning(
            'Problem getting number of steps ("{}"), e = {}'.format(result_type, e)
        )
...
def write_config_as_yaml(config: dict, dir_out: str, fname_out: str = "config.yaml"):
    ...
    if isinstance(config, omegaconf.dictconfig.DictConfig):
        logger.info(
            "Dumping the OmegaConf config to disk as .yaml ({})".format(path_out)
        )
        ...
        with open(path_out) as f:
            ...
            assert config == loaded_dict, (
                "The OmegaConf dictionary dumped to disk as .yaml "
                "is not the sane as used "
                "for training, something funky happened during saving"
            )
    else:
        raise NotImplementedError(
            "Dumping vanilla Python dictionary to disk as .yaml not implemented"
        )
