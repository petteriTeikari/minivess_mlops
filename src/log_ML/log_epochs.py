...
def log_epoch_for_tensorboard(
    train_epoch_results, eval_epoch_results, epoch, cfg, output_dir, output_artifacts
):
    # Create the tensorboard_log subdirectory for this repeat (submodel of inference)
    output_artifacts["epoch_level"]["tb_dir"] = os.path.join(
    ...
    try:
        ...
        logger.warning(
            'Problem creating Tensorboard artifact directory to "{}",'
    ...
    if "tb_writer" not in list(output_artifacts["epoch_level"].keys()):
        logger.info(
            'Logging Tensorboard epoch-by-epoch results to dir = "{}"'.format(
                output_artifacts["epoch_level"]["tb_dir"]
            )
        )
        output_artifacts["epoch_level"]["tb_writer"] = SummaryWriter(
            log_dir=output_artifacts["epoch_level"]["tb_dir"]
        )

    # Log training split (these are the batch-level metrics)
    output_artifacts = log_epoch_tb_metrics(
        results_dict=train_epoch_results,
    ...
    output_artifacts = log_epoch_tb_metrics(
        results_dict=eval_epoch_results,
        out_dir=output_artifacts["epoch_level"]["tb_dir"],
        output_artifacts=output_artifacts,
        epoch=epoch,
        cfg=cfg,
...
def log_epoch_tb_metrics(
    ...
    if metric_type == "train":
        split_name = "TRAINING"
        output_artifacts = log_epoch_tb_per_split(
            split_dict=results_dict,
        ...
        for split_name in results_dict.keys():
            output_artifacts = log_epoch_tb_per_split(
                split_dict=results_dict[split_name],
                split_name=split_name,
                metric_type=metric_type,
                epoch=epoch,
                cfg=cfg,
        ...
        raise IOError(
            'Unknown metric_type={}!, should be either "train" or "evaluation"'.format(
                metric_type
            )
        )
...
def log_epoch_tb_per_split(
    ...
    for dataset_name in dataset_names:
        output_artifacts = log_epoch_per_split_per_database(
            dataset_dict=split_dict[dataset_name],
            split_name=split_name,
            metric_type=metric_type,
...
def log_epoch_per_split_per_database(
    dataset_dict: dict,
    split_name: str,
    metric_type: str,
    dataset_name: str,
    ...
    for var_type in variable_types:
        if var_type == "scalars":
            # e.g. loss per epoch
            output_artifacts = log_scalars(
                metric_dict=dataset_dict[var_type],
                var_type=var_type,
                split_name=split_name,
                metric_type=metric_type,
                dataset_name=dataset_name,
            ...
            raise NotImplementedError(
                'Unknown variable_type="{}", a typo or you added some new type in addition to'
