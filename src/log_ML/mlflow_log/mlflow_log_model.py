...
def log_model_name(metamodel_name: str,
                   hyperparam_name: str,
...
def get_the_size_of_code_path(code_paths: list,
    ...
    logger.info('Total size of code_paths to be logged with pyfunc model = {:.2f} kB'.format(file_sum))
    if file_sum/1024 > alert_threshold_MB:
        logger.warning('Total size ({:.1f} MB) of code_paths is larger than {} MB, '
                       'are you trying to include "too much files"'.format(file_sum/1024, alert_threshold_MB))
...
def mlflow_metamodel_logging(ensemble_model,
    ...
    artifact_path = log_model_name(metamodel_name=metamodel_name,
                                   hyperparam_name=run_params_dict['hyperparam_name'],
    ...
    logger.info('MLflow | Model logging to Models done in {:.1f} seconds'.format(time.time() - t0))
