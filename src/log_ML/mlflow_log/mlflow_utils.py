...
def get_mlflow_model_signature_from_dataloader_dict(
    model_in: ModelEnsemble, experim_dataloaders: dict
):
    """
    ...
    batch_data = get_first_batch_from_dataloaders_dict(
        experim_dataloaders=experim_dataloaders
    )

    # Inferring the input_output relationship
    model_output = define_model_input_and_output_shapes(
        image_tensor=batch_data["image"], model_in=model_in, return_mask=True
    )

    # Define the signature
    signature = define_model_schema(
        model_input=batch_data["image"], model_output=model_output
    )
...
def define_model_schema(model_input, model_output):
    ...
    params_schema = ParamSchema([ParamSpec("return_mask", "boolean", True)])
...
def define_model_input_and_output_shapes(
    image_tensor, model_in: ModelEnsemble, return_mask: bool = False
):
    tensor_input = convert_to_tensor(image_tensor)
    numpy_input = tensor_input.numpy()
    model_output: dict = model_in.predict(
        None, model_input=numpy_input, param={"return_mask": return_mask}
    )
