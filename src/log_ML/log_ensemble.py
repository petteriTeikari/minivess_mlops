...
def log_ensemble_results(
    ...
    if service is not None:
        logger.info(
            "Logging ensemble metrics to experiment tracking service = {}".format(
                service
            )
        )
        for split in ensemble_results:
            for ensemble_name in ensemble_results[split]:
                ...
                for metric in metrics:
                    ...
                    metric_name = "{}/ensemble_{}/{}/{}".format(
                        fold_name, metric, split, ensemble_name
                    )
                    ...
                    if service == "MLflow":
                        ...
                        raise NotImplementedError(
                            'Unknown Experiment Tracking service = "{}"'.format(service)
                        )
        ...
        logger.warning(
            "Not logging ensemble metrics as no Experiment Tracking services enabled".format(
                service
            )
        )
