...
def log_wandb_repeat_results(
    fold_results: dict, output_dir: str, cfg: dict, log_models_with_repeat: bool = False
):
    logger.info(
        "Logging repeat-wise results to WANDB (local_dir = {})".format(output_dir)
    )
    for f, fold_name in enumerate(fold_results):
        for a, archi_name in enumerate(fold_results[fold_name]):
            for r, repeat_name in enumerate(fold_results[fold_name][archi_name]):
                ...
                try:
                    wandb_run = wandb_init_wrapper(
                        project=cfg_key(cfg, "run", "ARGS", "project_name"),
                        name=log_name,
                    ...
                    raise Exception(
                        "Problem with initializing Weights and Biases, e = {}".format(e)
                    )
                ...
                wandb_log_repeat(
                    ...
                    repeat_name,
                    log_name,
                    results=fold_results[fold_name][archi_name][repeat_name],
                    cfg=cfg,
...
def wandb_log_repeat(
    ...
    if log_models_with_repeat:
        ...
        wandb_log_models_from_model_dir(
            wandb_run, artifact, model_path=results["output_artifacts"]["model_dir"]
        )
        ...
        logger.info(
            "Skipping model artifact logging at repeat-level logging! Logging later with CV ensemble"
        )
...
def log_wandb_ensemble_results(ensembled_results: dict, output_dir: str, cfg: dict):
    ...
    for f, fold_name in enumerate(ensembled_results):
        ...
        wandb_log_ensemble_per_fold(
            wandb_run,
            fold_name,
            log_name,
            results=ensembled_results[fold_name],
            cfg=cfg,
        )
        ...
        logger.info(
            "Logging the ensemble output directory to WANDB: {}".format(dir_out)
        )
...
def log_crossval_res(
    ...
    if len(logging_services) == 0:
        logger.warning(
            "No logging (Experiment tracking such as MLflow or WANDB) "
        ...
        model_paths = log_cv_ensemble_results(
...
def log_cv_results(
    ...
    metrics_to_metadata: tuple = (
        "time_",
    ),  # quick n dirty method to keep times from metrics
                ...
                for ds_eval in cv_results[dataset][tracked_metric][split]:
                    for var_type in cv_results[dataset][tracked_metric][split][ds_eval]:
                        if var_type in var_types:
                            for var in cv_results[dataset][tracked_metric][split][
                                ds_eval
                            ][var_type]:
                                for stat_key in cv_results[dataset][tracked_metric][
                                    split
                                ][ds_eval][var_type][var]:
                                    if stat_key not in stat_keys_to_reject:
                                        v = cv_results[dataset][tracked_metric][split][
                                            ds_eval
                                        ][var_type][var][stat_key]
                                        metric_name = "CV_{}/{}/{}_{}".format(
                                            split, dataset, var, stat_key
                                        )
    ...
    if "WANDB" in logging_services:
        logger.info(
            "WANDB | Logging the directory {} as an artifact".format(cv_dir_out)
        )
        artifact = wandb.Artifact(name=log_name, type="artifacts")
...
def log_cv_metric(
    ...
    if log_this_metric:
        ...
        logger.debug(
            'SKIP THIS: {} | "{}": {:.3f}'.format(logging_services, metric_name, value)
        )


def log_cv_ensemble_results(
    cv_ensemble_results: dict,
    ensembled_results: dict,
    cv_dir_out: str,
    fold_results: dict,
            ...
            for metric in cv_ensemble_results[split][ensemble_name]:
                for stat_key in cv_ensemble_results[split][ensemble_name][metric]:
                    if stat_key not in stat_keys_to_reject:
                        ...
                        stat_dict = cv_ensemble_results[split][ensemble_name][metric][
                            stat_key
                        ]
                        value = stat_dict[stat_key2]
                        metric_name = "CV-ENSEMBLE_{}/{}/{}_{}".format(
                            split, ensemble_name, metric, stat_key
                        )

                        log_cv_ensemble_metric(
                            logging_services=logging_services,
                            metric_name=metric_name,
                            value=value,
                            cfg=cfg,
    ...
    model_paths = log_config_artifacts(
        log_name=log_name,
...
def log_cv_ensemble_metric(logging_services: list, metric_name: str, value, cfg: dict):
    logger.info('{} | "{}": {:.3f}'.format(logging_services, metric_name, value))
    if "WANDB" in logging_services:
        wandb.log({metric_name: value}, step=0)
...
def log_main_metrics(metric_name: str, value: float, logging_services: list, cfg: dict):
    ...
    if metric_out is not None:
        logger.info(
            '{} (MAIN) | "{}": {:.3f}'.format(logging_services, metric_out, value)
        )
        if "WANDB" in logging_services:
...
def wandb_log_metrics_of_repeat(wandb_run, results):
    no_train_batches = get_number_of_steps_from_repeat_results(
        results, result_type="train_results"
    )
    no_train_epochs = get_number_of_steps_from_repeat_results(
        results, result_type="eval_results"
    )
    wandb_log_array(
        res=results["eval_results"],
        result_type="eval_results",
        no_steps=no_train_epochs,
    )


def wandb_log_models_from_model_dir(
    wandb_run, artifact, model_path: str, model_ext: str = "*.pth"
):
...
def wandb_log_ensemble_per_fold(
    wandb_run,
    fold_name: str,
    log_name: str,
    results: dict,
            ...
            for metric in ensemble_metrics:
                ...
                for var_key in value_dict:
                    if var_key in var_keys:
                        ...
                        metric_name = "ENSEMBLE_{}/{}/{}_{}".format(
                            split, ensemble_name, metric, var_key
                        )
