...
from src.log_ML.bentoml_log.bentoml_utils import (
    parse_tag_from_exception,
    get_bento_CLI_build_command,
)
from src.utils.config_utils import get_repo_dir
from src.utils.dict_utils import cfg_key


def export_bento_to_s3(model_name_in: str, model_name_out: str, s3_bucket: str):
...
def get_latest_build_tag():
    ...
    logger.info(
        "Latest Bento tag = {} (created {})".format(
            latest_bento_tag, latest_bento.creation_time
        )
    )
...
def log_bento_built_stdout(stdout: str):
    ...
    if not line_found:
        logger.warning(
            'Bento build output maybe changed as could not find the line "Successfully built Bento"'
        )


def bentoml_save_bento(
    bento_svc_cfg: DictConfig, export_to_s3: bool = True, run_id: str = None
):
