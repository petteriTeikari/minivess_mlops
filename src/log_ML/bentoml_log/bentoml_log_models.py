...
def build_and_export_bento_to_s3(model_name_in: str,
                                 model_name_out: str,
                                 s3_bucket: str):
    ...
    # And export
    export_bento_to_s3(model_name_in=model_name_in,
...
def export_bento_model_to_s3(model_name_in: str,
                       model_name_out: str,
                       s3_bucket: str):

    # https://docs.bentoml.org/en/latest/concepts/bento.html#bento-management-apis
    logger.info('Export Bento Model to S3 bucket = {}'.format(s3_bucket))
    s3_path = bentoml.models.export_model(model_name_in, f'{s3_bucket}/{model_name_out}')
    logger.info('Logged to path = {}'.format(s3_path))


def import_bento_model_from_s3(model_name: str,
                               s3_bucket: str,
                               model_tag: str = 'latest'):
    ...
    try:
        ...
        tag = parse_tag_from_exception(str(e))
        logger.warning('Failed to import Bento from S3 (tag = {}), e = {}'.format(tag, e))
        try:
            # try to load locally then
    ...
    logger.debug('Bento Model, creation time = {}'.format(bento_model_import.creation_time))
...
def bentoml_save_mlflow_model_to_model_store(run: mlflow.entities.Run,
    ...
    if s3_export:
        export_bento_model_to_s3(model_name_in=cfg_key(bento_svc_cfg, 's3_model_name'),
