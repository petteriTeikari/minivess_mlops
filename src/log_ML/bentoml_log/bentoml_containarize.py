...
def log_docker_build_std_out_err(stdout: str,
                                 stderr: str):
    logger.info(stderr)
    logger.info(stdout)
...
def modify_docker_image_tags(docker_name: str,
                             tag: str,
                             docker_image: str,
                             run_id: str = None):
    """
    ...
    logger.info('Adding another tag (MLFlow run_id) to the Docker image, cmd = "{}"'.format(cmd))
    out = subprocess.run(cmd, capture_output=True, shell=True, text=True)
...
def push_bento_to_docker_hub(docker_image: str):
    ...
    logger.info('Bento Docker upload to Docker Hub took {:.0f} seconds'.format(time.time() - t0))


def containarize_bento(bento_tag: str,
                       docker_image: str,
                       run_id: str,
                       no_cache: bool = True):
    """
    ...
    t0 = time.time()
    out = subprocess.run(cmd, capture_output=True, shell=True, text=True)
    docker_name, tag = log_docker_build_std_out_err(stdout=out.stdout,
                                                    stderr=out.stderr)
    ...
    modify_docker_image_tags(docker_name=docker_name,
                             tag=tag,
