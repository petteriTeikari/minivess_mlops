...
def log_cv_results(
    ...
    logging_services = get_used_services(
        logging_cfg=cfg_key(cfg, "hydra_cfg", "config", "LOGGING")
    )
...
def save_log_cv_dicts_to_disk(
    ...
    cv_ensemble_path_out = os.path.join(
        cv_ensembled_output_dir, "cv_ensemble_results.json"
    )
    json_object2 = json.dumps(cv_ensemble_results, default=to_serializable)
    with open(cv_ensemble_path_out, "w") as outfile:
        outfile.write(json_object2)
    logger.info(
        "Wrote CV Ensemble results to disk as .JSON ({})".format(cv_ensemble_path_out)
    )
