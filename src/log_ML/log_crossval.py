...
from src.log_ML.log_utils import convert_value_to_numpy_array
...
def reorder_crossvalidation_results(fold_results: dict):
    ...
    for f, fold_key in enumerate(fold_results):
        ...
        no_of_repeats = len(fold_result)  # same as number of submodels in an ensemble
                        ...
                        for d_e, ds_name_eval in enumerate(repeat_best_eval[split]):
                            ...
                            for t, var_type in enumerate(eval_metrics):
                                ...
                                for v, var_name in enumerate(eval_metrics[var_type]):
                                    ...
                                    if var_name not in res_out[ds_name][metric][split][ds_name_eval][var_type].keys():
                                        value_array = np.zeros((no_of_folds, no_of_repeats))
                                        res_out[ds_name][metric][split][ds_name_eval][var_type][var_name] = value_array

                                    # res_out_tmp = res_out[ds_name][metric][split][ds_name_eval][var_type][var_name]
                                    res_out[ds_name][metric][split][ds_name_eval][var_type][var_name][f,r] = value_in
                    ...
                    for t, var_type in enumerate(eval_metrics):
                        ...
                        for v, var_name in enumerate(eval_metrics[var_type]):
                            ...
                            res_out[ds_name][metric][split][ds_name_eval][var_type][var_name] = (
                                compute_cv_stats(value_array_in))

    return res_out


def compute_cv_stats(value_array_in: np.ndarray):
