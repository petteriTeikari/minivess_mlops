...
from src.log_ML.bentoml_log.bentoml_log_models import (
    bentoml_save_mlflow_model_to_model_store,
)
...
from src.log_ML.mlflow_log.mlflow_utils import (
    get_mlflow_model_signature_from_dataloader_dict,
)
...
def log_ensembles_to_mlflow(
    ...
    for i, ensemble_name in enumerate(ensemble_models_flat):
        if i == 0:
            ...
            ensemble_models[ensemble_name] = ModelEnsemble(
                models_of_ensemble=ensemble_models_flat[ensemble_name],
                models_from_paths=True,
                validation_config=cfg_key(cfg, "hydra_cfg", "config", "VALIDATION"),
                ensemble_params=cfg_key(
                    cfg, "hydra_cfg", "config", "ENSEMBLE", "PARAMS"
                ),
            ...
            try:
                signature = get_mlflow_model_signature_from_dataloader_dict(
                    model_in=ensemble_models[ensemble_name],
                    experim_dataloaders=experim_dataloaders,
                )
            except Exception as e:
                logger.warning(
                    "Problem creating MLflow ModelSignature, e = {}".format(e)
                )
                signature = None

            # Log the model to Models (and register it to Model Registry)
            mlflow_model_log[ensemble_name] = mlflow_metamodel_logging(
            ...
            if test_loading:
                ...
                test_config = cfg_key(
                    cfg, "hydra_cfg", "config", "LOGGING", "MLFLOW", "TEST_LOGGING"
                )
                test_results = test_mlflow_models_reproduction(
                    ensemble_filepaths=ensemble_models_flat[ensemble_name],
                    ensemble_model=ensemble_models[ensemble_name],
            ...
            logger.warning(
                "At the moment the Model Registry is working only for single metamodel per run\n"
...
def log_ensembles_to_wandb(
    ...
    for i, ensemble_name in enumerate(ensemble_models_flat):
        for j, submodel_name in enumerate(ensemble_models_flat[ensemble_name]):
            ...
            logger.info(
                "WANDB | Model file logged to registry: {}".format(artifact_name)
            )
...
def register_model_from_run(
    ...
    if "mlflow" in services:
        ...
        try:
            reg, model_uri = mlflow_register_model_from_run(
                run=run, stage=stage, project_name=project_name
            )
            ...
            try:
                ...
                if "bentoml" in services:
                    ...
                    bento_svc_cfg = cfg_key(
                        cfg, "hydra_cfg", "config", "SERVICES", "BENTOML"
                    )
                    ...
                    bento = bentoml_save_bento(
                        bento_svc_cfg=bento_svc_cfg, run_id=run_id
                    )
                ...
                logger.error(
                    "Failed to save the model to BentoML Model Store, e = {}".format(e)
                )
                raise IOError(
                    "Failed to save the model to BentoML Model Store, e = {}".format(e)
                )
            ...
            logger.error(
                "Failed to register the model to MLflow Model Registry, e = {}".format(
                    e
                )
            )
            raise IOError(
        ...
        logger.info(
            "Model had improved, but MLflow Model Registry model registration is skipped"
        )
