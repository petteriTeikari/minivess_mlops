...
def import_mlflow_model(mlflow_model: dict):
    ...
    try:
        ...
        logger.error('Fail to get the download URI for the MLflow model! e = {}'.format(e))
        raise IOError('Fail to get the download URI for the MLflow model! e = {}'.format(e))
...
def load_model_from_registry(model_uri: str):
    ...
    try:
        ...
        logger.error('Loading registered model from "{}" failed, e = {}'.format(model_uri, e))
        raise IOError('Loading registered model from "{}" failed, e = {}'.format(model_uri, e))
