...
def ml_test_filelisting_corruption(
    list_of_files: list, import_library: str = "nibabel", debug_verbose: bool = False
):
    info_of_files = []
    ...
    for i, filepath in enumerate(list_of_files):
        if import_library == "nibabel":
            info_out, read_problem = nibabel_file_checker(
                filepath=filepath, debug_verbose=debug_verbose
            )
            info_of_files.append(info_out)
            read_problem_of_files.append(read_problem)
        else:
            raise NotImplementedError(
                'Unknown file import library = "{}"'.format(import_library)
            )
    ...
    problematic_files = list(
        compress(read_problem_of_files, idx_for_filenames_with_problems)
    )
...
def nibabel_summary_of_all_files(info_of_files: list, read_problem_of_files: list):
    ...
    def get_size_stats_per_coord(list_of_sizes: list) -> dict:
        return {
            "mean": float(np.mean(list_of_sizes)),
    ...
    def print_size_stats(size_stats) -> None:
        ...
        for key_in in size_stats.keys():
            ...
            if key_in == "z":
                logger.info(
                    "When you are applying crops, "
                    "your minimum z size is = {} voxels, "
                    "and thus your crops must be smaller than this in z-dimension".format(
                        size_stats[key_in]["min"]
                    )
                )
...
def nibabel_file_checker(filepath: str, debug_verbose: bool = False):
    ...
    try:
        ...
        dict_out = {"n1_header": n1_header, "volume_shape": n1_img.shape}
        if debug_verbose:
