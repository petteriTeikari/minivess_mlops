...
def ml_test_dataloader_dict_integrity(
    dataloaders: dict, test_config: dict, run_params: dict, debug_testing: bool = True
):
    ...
    for fold_name in dataloaders:
        ...
        for split_name in fold:
            (
                dataloader_metrics[fold_name][split_name],
                dataloader_ok_dict[fold_name][split_name],
            ) = ({}, {})
    ...
    all_tests_ok, report_string = dataloader_test_summary(
        dataloader_metrics=dataloader_metrics, dataloader_ok_dict=dataloader_ok_dict
    )

    if not all_tests_ok:
        logger.debug(
            "Here is your chance to write the report string to disk as well as .txt"
        )
        print(report_string)


def dataloader_test_summary(
    dataloader_metrics: dict,
    ...
    for fold_name in dataloader_metrics:
        ...
        for split_name in dataloader_metrics[fold_name]:
            ...
            for dataset in dataloader_metrics[fold_name][split_name]:
                ...
                report_string, all_tests_ok = add_boolean_and_metric_strings_to_summary(
                    metrics=metrics,
                    booleans=booleans,
...
def ml_test_single_dataloader(
    ...
    (
        dataloader_ok_dict["dataset_has_valid_types"],
        dataloader_metrics["dataset_has_valid_types"],
    ) = ml_test_dataloader_for_allowed_types(
    ...
    if dataloader_ok_dict["dataset_has_valid_types"]:
        ...
        for i, batch_data in enumerate(dataloader):
            logger.info(
                "ML Test for batch {}/{} in split = {}, fold = {}, dataset = {}".format(
                    i + 1, no_batches, split_name, fold_name, dataset
                )
            )

            batch_metrics, batch_ok_dict = ml_test_single_batch_dict(
                batch_data,
                test_config=test_config,
                split_name=split_name,
                fold_name=fold_name,
            ...
            dataloader_metrics, dataloader_ok_dict = dataloader_collector_of_batches(
                dataloader_metrics,
                dataloader_ok_dict,
                batch_metrics,
                batch_ok_dict,
        ...
        logger.warning(
            "Cannot run nan/inf checks as you have illegal entries in the dataset(s) of your dataloader"
        )

    return dataloader_metrics, dataloader_ok_dict


def dataloader_collector_of_batches(
    dataloader_metrics,
    dataloader_ok_dict,
    batch_metrics,
    batch_ok_dict,
    ...
    if len(dataloader_metrics) == 0:
        ...
        if add_dummy_data:
            ...
            batch_metrics["image_nans_infs"][
                "dummy{}".format(random.randint(3, 22))
...
def ml_test_single_batch_dict(
    ...
    batch_test_metrics[test_name], batch_tests[test_name] = ml_test_batch_nan_and_inf(
        batch_tensor=batch_data["image"],
        test_name=test_name,
        filenames=filenames,
...
def check_batch_type(batch_data: dict):
    if not isinstance(batch_data["image"], torch.Tensor):
        logger.error(
            "Your batch data is not a torch.Tensor, but {}", type(batch_data["image"])
        )
        logger.error(
            "This could happen when using the vanilla Dataset when the paths to files are here"
        )
        # Implement this if needed, but you might as well can use the CachedDataset with cache rate of 0.0
        raise IOError(
            "Your batch data is not a torch.Tensor, but {}".format(
                type(batch_data["image"])
            )
        )


def ml_test_batch_nan_and_inf(
    batch_tensor: monai.data.MetaTensor,
    test_name: str,
    filenames: list,
    fake_a_nan: bool = False,
):
    batch_test_metrics = {}
    assert len(batch_tensor.shape) == 5, (
        "Only 5D input tensors supported now" "(batch, channels, x, y, z)"
    )
        ...
        for sample_idx in range(len(if_sample_has_nans)):
            ...
            if if_sample_has_nans[sample_idx]:
                ...
                if error_name not in batch_test_metrics:
                    batch_test_metrics = get_per_sample_naninf_stats(
                        batch_test_metrics,
...
def get_per_sample_naninf_stats(
    batch_test_metrics: dict,
    error_name: str,
    ...
    batch_test_metrics[fname_wo_ext]["percentage"] = 100 * (
...
def ml_test_dataloader_for_allowed_types(
    ...
    try:
        ...
        try:
            is_dataset_valid, samples_not_valid = ml_test_dataset_for_allowed_types(
                split_file_dict=dataloader.dataset.data
            )
            assert dataloader_is_valid == is_dataset_valid, "these should be the same!"
        except Exception as e:
            raise IOError(
                'Failed to get the "split_file_dict" Some new type of dataloader? e = {}'.format(
                    e
                )
            )
