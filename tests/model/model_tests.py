...
from src.utils.model_utils import (
    get_nested_layers,
    filter_layer_names,
    create_pseudomodel_from_filtered_layers,
)


def model_tests_main(
    model,  # after epoch
    ...
    if test_config[test_type]["enable"]:
        ...
        model_test_metrics[test_type] = weightwatchers_tests(
            model, first_epoch=first_epoch
        )
...
def weightwatchers_tests(
    ...
    if output_dummy_output:
        ...
        if len(conv_layers) > 0:
            ...
            if first_epoch:
                logger.info(
                    "{} supported convolution layers found for WeightWarcher "
                    "(note! does not support 3D convolutions atm)".format(
                        len(conv_layers)
                    )
                )
            ...
            details = (
                watcher.analyze()
            )  # nothing outputted for conv2d even, TODO! examine this
            summary = watcher.get_summary(details)

        else:
            if first_epoch:
                logger.info(
                    'No convolution layers (wildcard = "{}") found for WeightWarcher '
                    "(note! does not support 3D convolutions atm)".format(
                        layer_name_wildcards
                    )
                )
