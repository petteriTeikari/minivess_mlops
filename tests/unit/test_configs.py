...
def get_config_files(
    cfg_dir: str, exclude_files: tuple = ("defaults.yaml"), extension: str = "yaml"
):
        ...
        for file in files:
            if file.endswith(extension):
                if file not in exclude_files:
                    ...
                    rel_path = abs_path.replace(cfg_dir + os.sep, "").replace(
                        "." + extension, ""
                    )
...
def test_import_configs(cfg_dir: str = None, exclude_files: tuple = ("defaults.yaml",)):
    if cfg_dir is None:
        cfg_dir = CFG_DIR
    else:
        if not os.path.exists(cfg_dir):
    ...
    try:
        abs_config_files, relative_cfg_paths = get_config_files(
            cfg_dir=cfg_dir, exclude_files=exclude_files
        )
    ...
    try:
        ...
        for defaults_cfg in exclude_files:
            for relative_cfg_path in relative_cfg_paths:
                i += 1
                logger.info(
                    "#{}: Testing cfg = {} (base = {})".format(
                        i, relative_cfg_path, defaults_cfg
                    )
                )
                try:
                    config = config_import_script(
                        task_cfg_name=relative_cfg_path,
                    ...
                    logger.error(
                        'Fail to import config "{}", e = {}'.format(
                            relative_cfg_path, e
                        )
                    )
