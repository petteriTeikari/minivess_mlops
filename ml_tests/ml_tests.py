...
# of being scattered around in the .py files in "ml_tests" folder

def ml_test_data_not_corrupted(corrupted_files: list) -> bool:
    assert len(corrupted_files) == 0, 'The following files were corrupted: {}'.format(corrupted_files)