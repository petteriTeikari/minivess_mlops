...
from src.log_ML.bentoml_log.bentoml_log_models import import_bento_from_s3, get_mlflow_run_id_of_bento, \
...
def parse_args_to_dict():
    ...
    parser.add_argument('-mlflow_log', '--mlflow_model_name', type=str, required=False,
                        default='minivess-test2',
                        help="The name that you used to log models to MLflow Model Registry")
    parser.add_argument('-bentoml_log', '--bentoml_model_name', type=str, required=False,
                        default='minivess-segmentor',
                        help="The name that you used to log model to BentoML Model Store")
    parser.add_argument('-o', '--output', type=str, required=False,
                        default='is_docker_old', choices=['is_docker_old'],
                        help="The name that you used to log model to BentoML Model Store")
...
def compare_mlflow_and_bento(mlflow_model, bento_model):
    ...
    if bento_model is not None:
        ...
        logger.info('MLFlow and BentoML model run_ids match, run_id = {}'.format(mlflow_run_id))
    else:
        logger.warning('Could not import the latest Bento model, '
...
if __name__ == '__main__':
    ...
    tracking_uri, env_vars_set = initialize_mlflow(server_uri=cfg_key(service_uris, 'MLFLOW', 'server_URI'))
    mlflow_model = import_best_model_from_model_registry(model_name=args['mlflow_model_name'],
                                                         env_vars_set=env_vars_set,
                                                         tracking_uri=tracking_uri)

    # get the latest Bento Model from the S3 bucket
    bento_model = import_bento_model_from_s3(model_name=args['bentoml_model_name'],
    ...
    docker_built_from_best_run_id = (
        get_latest_bento_docker(docker_image=cfg_key(service_uris, 'BENTOML', 'docker_image'),
