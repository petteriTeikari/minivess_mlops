...
def import_yaml_file(yaml_path: str):
    ...
    if "cfg" not in locals():
        raise IOError(
            "YAML import failed! See the the line and columns above that were not parsed correctly!\n"
    ...
    for d, dataset in enumerate(repeat_result_example):
        ...
        for m, metric_to_track in enumerate(repeat_result_example[dataset]):
            ...
            ensemble_name = get_ensemble_name(
                dataset_validated=dataset, metric_to_track=metric_to_track
            )
            ...
            with torch.no_grad():
                for batch_idx, batch_data in enumerate(
                    ...
                    ensemble_stat_results = ensemble_repeats(
                        inf_res=inference_results, cfg=cfg
                    )
            ...
            split_metrics_stat[dataset][metric_to_track] = compute_split_metric_stats(
                split_metrics_tmp=split_metrics[dataset][metric_to_track]
            )
...
def inference_dataloader(dataloader, model, device, split: str, config: dict):
    ...
    with torch.no_grad():
        for batch_idx, batch_data in enumerate(
            tqdm(
                dataloader,
                desc='BEST REPEAT: Inference on dataloader samples, split "{}"'.format(
                    split
                ),
            )
        ):
            ...
            metrics = get_inference_metrics(
                y_pred=sample_res["arrays"]["binary_mask"],
                eval_cfg=cfg["config"]["VALIDATION_BEST"],
                batch_data=batch_data,
            )

            split_metrics = add_sample_metrics_to_split_results(
                sample_metrics=deepcopy(metrics), split_metrics_tmp=split_metrics
            )
...
def log_ensembles_to_MLflow(
    ensemble_models_flat: dict,
    ...
    for i, ensemble_name in enumerate(ensemble_models_flat):
        logger.info(
            "Ensemble #{}/{} | ensemble_name = {}".format(
                i + 1, len(ensemble_models_flat), ensemble_name
            )
        )
        ...
        for j, submodel_name in enumerate(ensemble_models_flat[ensemble_name]):
            ...
            logger.info(
                "Submodel #{}/{} | local_path = {}".format(
                    j + 1, no_submodels_per_ensemble, model_path
                )
            )
            ...
            torch.save(
                dict(model=model), "local_{}.pth".format(submodel_name)
            )  # debug save

            # Log the model (and register it to Model Registry)
            mlflow_model_log[ensemble_name][submodel_name] = mlflow_model_logging(
                model=model,
                best_dict=best_dict,
                model_uri=model_uri,
        ...
        if test_loading:
            ...
            logger.info(
                "MLflow | Test that you can download model from the "
                "Model Registry and that they are reproducible"
            )
            test_mlflow_model_registry_load(
                ensemble_submodels=ensemble_models_flat[ensemble_name],
            ...
            logger.warning(
                "MLflow | Skipping the model loading back from MLflow, are you sure?\n"
...
def mlflow_single_model_logging(
    model,
    best_dict: dict,
    model_uri: str,
    mlflow_config: dict,
    ...
    mlflow_model_log = {}
    t0 = time.time()
    artifact_name = define_artifact_name(
        ensemble_name, submodel_name, hyperparam_name=run_params_dict["hyperparam_name"]
    )
    ...
    mlflow_model_log["model_info"] = mlflow.pytorch.log_model(
        pytorch_model=model,
    ...
    mlflow_model_log["reg_model"] = mlflow.register_model(
        model_uri=model_uri,
        name=artifact_name,
    ...
    logger.info(
        "MLflow | Model log and and registering done in {:.3f} seconds".format(
            time.time() - t0
        )
    )
...
def create_model_ensemble_from_mlflow_model_registry(
    ensemble_submodels: dict,
    ensemble_name: str,
    config: dict,
    model_info: dict,
    best_dicts: dict,
    ...
    for j, submodel_name in enumerate(ensemble_submodels):
        ...
        model_uri_models = (
            f"models:/{artifact_name}/{reg_models[submodel_name].version}"
        )
        loaded_model = deepcopy(
            get_model_from_mlflow_model_registry(model_uri=model_uri_models)
        )
        torch.save(
            dict(model=loaded_model), "{}.pth".format(submodel_name)
        )  # debug save
...
def authenticate_mlflow(fname_creds: str = "mlflow_credentials.ini"):
    ...
    if not os.path.exists(secrets_dir):
        raise IOError(
            "Cannot found secrets folder ({}), thus cannot authenticate to MLflow".format(
                secrets_dir
            )
        )
    else:
        ...
        if not os.path.exists(credentials_file):
            raise IOError(
                "Cannot found credentials file ({}), "
                "thus cannot authenticate to MLflow".format(credentials_file)
            )
        else:
            # https://stackoverflow.com/a/8884638
            logger.info(
                " reading MLflow credentials from file '{}'".format(fname_creds)
            )
            credentials = configparser.ConfigParser()
            credentials.read(credentials_file)

    # I guess you could have slightly nicer weay to handle sensitive information
    os.environ["MLFLOW_TRACKING_USERNAME"] = credentials["mlflow_log"][
        "mlflow_tracking_username"
    ]
    os.environ["MLFLOW_TRACKING_PASSWORD"] = credentials["mlflow_log"][
